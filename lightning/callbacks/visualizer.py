...
import torch
from pytorch_lightning import loggers, Callback, LightningModule, Trainer
...
class PredandGTPlotCallback(Callback):  # pragma: no cover
    ...
    def _plot(
        ...
        if isinstance(trainer.logger, loggers.TensorBoardLogger):
            trainer.logger.experiment.add_figure(
                f"{mode}_examples", fig, global_step=trainer.global_step
            )

        elif isinstance(trainer.logger, loggers.wandb.WandbLogger):

            trainer.logger.experiment.log(
                {
                    f"{mode}/examples": [wandb.Image(fig)],
